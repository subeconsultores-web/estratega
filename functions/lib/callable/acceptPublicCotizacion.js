...
exports.acceptPublicCotizacion = void 0;
const functions = require("firebase-functions/v2");
const admin = require("firebase-admin");
exports.acceptPublicCotizacion = functions.https.onCall(async (request) => {
    const { id, firmaDato, opcionalesAceptados } = request.data;
    // firmaDato can contain { name, idNumber, ip } from the client
    if (!id || !(firmaDato === null || firmaDato === void 0 ? void 0 : firmaDato.name)) {
        throw new functions.https.HttpsError('invalid-argument', 'Se requiere el ID de la cotización y un Nombre válido para la firma electrónica.');
    }
    try {
        ...
        if (cotData.estadoActual !== 'Enviada' && cotData.estadoActual !== 'Revision_Solicitada') {
            throw new functions.https.HttpsError('failed-precondition', 'La cotización no se encuentra en un estado válido para ser aceptada públicamente.');
        }
        ...
        if (Array.isArray(opcionalesAceptados) && opcionalesAceptados.length > 0) {
            ...
            newSubtotal = 0;
            itemsActualizados = itemsActualizados.map((item, i) => {
        ...
        await cotRef.update({
            ...
            firmaElectronica: Object.assign(Object.assign({}, firmaDato), { fechaIP: admin.firestore.Timestamp.now() }),
...
//# sourceMappingURL=acceptPublicCotizacion.js.map