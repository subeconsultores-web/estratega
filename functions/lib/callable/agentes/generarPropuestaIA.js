...
exports.generarPropuestaIA = void 0;
const https_1 = require("firebase-functions/v2/https");
const admin = require("firebase-admin");
const genai_1 = require("@google/genai");
const params_1 = require("firebase-functions/params");
const geminiApiKey = (0, params_1.defineSecret)("GEMINI_API_KEY");
exports.generarPropuestaIA = (0, https_1.onCall)({
    cors: true,
    ...
    if (!clienteId || !serviciosOfrecidos) {
        throw new https_1.HttpsError('invalid-argument', 'Faltan parámetros requeridos (clienteId, serviciosOfrecidos).');
    }
    try {
        ...
        const contacto = ((_a = dataCliente.contactoPrincipal) === null || _a === void 0 ? void 0 : _a.nombre) || 'Representante';
        // Iniciamos el modelo
...
Atención a: ${contacto}
Servicios a ofrecer:
${serviciosOfrecidos.map((s) => `- ${s.nombre} (Valor ref: ${s.precio})`).join('\\n')}
...
Contexto adicional: ${contextoExtra || 'Elabora argumentos de venta convincentes para estos servicios.'}`;
        ...
        // Limpieza básica
        if (propuestaMarkdown.startsWith('```markdown'))
            propuestaMarkdown = propuestaMarkdown.replace(/^```markdown\\n?/, '');
        if (propuestaMarkdown.startsWith('```'))
            propuestaMarkdown = propuestaMarkdown.replace(/^```\\n?/, '');
        if (propuestaMarkdown.endsWith('```'))
            propuestaMarkdown = propuestaMarkdown.replace(/\\n?```$/, '');
        ...
        throw new https_1.HttpsError('internal', 'No se pudo generar la propuesta.', error.message);
    }
});
//# sourceMappingURL=generarPropuestaIA.js.map