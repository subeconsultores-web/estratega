...
export const forecastPredictivo = onCall({
    cors: true,
    secrets: [geminiApiKey]
}, async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'Autenticación requerida para usar Forecast IA.');
    }

    const { tenantId } = request.data;
    if (!tenantId) {
        throw new HttpsError('invalid-argument', 'El tenantId es requerido.');
    }
    ...
    try {
        ...
        const facturasDetails: any[] = [];
        facturasSnapshot.forEach(doc => {
            const data = doc.data();
            montoFacturasPendientes += (data.total || 0);
            facturasDetails.push({ id: doc.id, total: data.total, fechaVencimiento: data.fechaVencimiento?.toDate() });
        });
...
Genera el JSON de Forecast Predictivo.`;
        ...
        throw new HttpsError('internal', 'Error generando predictivo.', error.message);
    }
});
