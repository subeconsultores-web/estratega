...
export const generarSugerenciasUpselling = onCall({
    secrets: [geminiApiKey]
}, async (request) => {
    // 1. Verificación de Autenticación
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'El usuario debe estar autenticado para generar sugerencias.');
    }

    const tenantId = request.auth.token.tenantId;
    if (!tenantId) {
        throw new HttpsError('permission-denied', 'El usuario no tiene un tenantId asignado.');
    }
...
No incluyas markdown, código, ni texto adicional. Solo el JSON válido.`;
        ...
        let jsonRaw = response.text?.trim() || '{"sugerencias": []}';
            ...
            for (const sugerencia of upsellingResult.sugerencias) {
                ...
                if (sugerencia.clienteId) {
                    ...
                    batch.set(opRef, {
                        ...
                        estado: 'pendiente' // pendiente, contactado, descartado, ganado
                    });
                    generados++;
                }
            }
        ...
        throw new HttpsError('internal', 'Ocurrió un error al generar oportunidades médicas/comerciales de IA.');
    }
});
