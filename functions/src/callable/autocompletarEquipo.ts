...
interface UsuarioCandidate {
    uid: string;
    nombre: string;
    email: string;
    skills: string[];
    costoHora: number;
    cargaActualHoras: number;
    matchScore: number; // 0-100
}

export const autocompletarEquipo = onCall({
    cors: true,
    memory: '256MiB',
    maxInstances: 10
}, async (request) => {
    ...
    if (!proyectoId || !skillsRequeridos || !Array.isArray(skillsRequeridos) || skillsRequeridos.length === 0) {
        throw new HttpsError('invalid-argument', 'Se requiere proyectoId y un array de skillsRequeridos.');
    }
    ...
    try {
        ...
        const candidatos: UsuarioCandidate[] = [];

        usersSnapshot.forEach(doc => {
            const user = doc.data();
            const userSkills: string[] = (user.skills || []).map((s: string) => s.toLowerCase().trim());
            const reqSkillsNorm = skillsRequeridos.map((s: string) => s.toLowerCase().trim());

            // Calculate match score
            const matchingSkills = reqSkillsNorm.filter((rs: string) => userSkills.includes(rs));
        ...
        throw new HttpsError('internal', 'Error procesando la recomendación del equipo IA.');
    }
});
