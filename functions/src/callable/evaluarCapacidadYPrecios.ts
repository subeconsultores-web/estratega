...
export const evaluarCapacidadYPrecios = onCall({
    cors: true,
    memory: '256MiB',
    maxInstances: 10
}, async (request) => {
    ...
    if (!tenantId) {
        throw new HttpsError('permission-denied', 'No tenant ID found in user claims.');
    }
    ...
    try {
        ...
        let recomendacion = '';
        let tipoAlerta: 'premium' | 'descuento' | 'neutral' = 'neutral';
        ...
        throw new HttpsError('internal', 'Hubo un error procesando el sistema de tarifas dinámicas.');
    }
});
