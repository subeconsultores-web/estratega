...
@Component({
    ...
    template: `
        <div class="max-w-3xl mx-auto space-y-6">
            <div class="mb-8">
                <h1 class="text-2xl font-bold text-txt-primary tracking-tight">Centro de Soporte</h1>
                <p class="text-txt-secondary mt-1 text-sm">¿Tienes alguna duda o incidencia? Escríbenos y un agente lo resolverá a la brevedad.</p>
            </div>

            <div class="bg-surface rounded-xl shadow-elevation-1 border border-border p-6">
                <form (ngSubmit)="enviarTicket()" #ticketForm="ngForm" class="space-y-6">
                    <div>
                        <label class="block text-sm font-medium text-txt-secondary mb-1">Asunto</label>
                        <input type="text" [(ngModel)]="ticket.asunto" name="asunto" required
                            placeholder="Ej. Problemas con el acceso al sistema mensual"
                            class="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors bg-base text-txt-primary placeholder:text-txt-muted">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-txt-secondary mb-1">Prioridad</label>
                        <select [(ngModel)]="ticket.prioridad" name="prioridad"
                            class="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors bg-base text-txt-primary">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-txt-secondary mb-1">Descripción de la incidencia</label>
                        <textarea [(ngModel)]="ticket.mensaje" name="mensaje" required rows="5"
                            placeholder="Describe paso a paso lo que ha ocurrido..."
                            class="w-full px-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors resize-y bg-base text-txt-primary placeholder:text-txt-muted"></textarea>
                    </div>
            ...
            <div *ngIf="successMessage" class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700/40 text-green-700 dark:text-green-400 px-4 py-3 rounded-lg flex items-center">
